    ...
    case Order of
        #{table_id := TableId} ->
            case ets:lookup(table_state_ets, TableId) of
                [{_, #{customer_id := CustomerId}}] ->
                    io:format("[machine_fsm] Delivering order ~p to customer ~p (table ~p)~n", [Order, CustomerId, TableId]),
                    customer_fsm:receive_order(CustomerId, Order);
                _ ->
                    io:format("[machine_fsm] Could not find customer for table ~p~n", [TableId])
            end;
        _ ->
            io:format("[machine_fsm] Invalid order format: ~p~n", [Order])
    end,
